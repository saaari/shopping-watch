...
`DISCORD_BOT_TOKEN` + `DISCORD_CHANNEL_ID` 시크릿을 등록하면, 지정한 채널에 메시지로 키워드를 관리할 수 있습니다. (30분마다 읽어서 반영)
...
도움
```
- 사이트 별칭: 번개 · 중고나라 · 메루카리 · 라쿠텐 · 야후쇼핑 · 아마존재팬 · 야후옥션 · 아마존 · 이베이
- 명령은 **다음 자동 실행(최대 30분 이내)** 에 반영되고, 봇이 ✅ 확인 답장을 보냅니다.
- 봇 토큰·채널 설정을 안 하면 이 기능만 꺼지고 알림은 정상 동작합니다.
